...
bool eval(string gt_dir, string result_dir, string report_dir){
...
int32_t main (int32_t argc, char *argv[]) {
  ...
  //cout << format("writing %05d: %.2f \n") % 720 % 40.23;
  //return 0;

  // we need 2 or 4 arguments!
  if (argc!=3) {
    cout << "Usage: ./eval_detection_bev_offline gt_dir result_dir" << endl;
    return 1;
  }
  ...
  system(("mkdir " + report_dir).c_str());

  // run evaluation
  if (eval(gt_dir, result_dir, report_dir)) {
    cout << format("Evaluation results saved to: %s\n") % report_dir.c_str();
  } else {
