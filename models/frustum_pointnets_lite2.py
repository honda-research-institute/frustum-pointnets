...
def get_instance_seg_lite_net(point_cloud, one_hot_vec,
...
def get_3d_box_estimation_lite_net(object_point_cloud, one_hot_vec,
                                 is_training, bn_decay, end_points):
...
def get_model(point_cloud, one_hot_vec, is_training, bn_decay=None):
    ...
    logits, end_points = get_instance_seg_lite_net(\
    ...
    output, end_points = get_3d_box_estimation_lite_net(\
...
if __name__=='__main__':
    ...
    with tf.Graph().as_default():
        inputs = tf.zeros((32,NUM_POINT,4))
        outputs = get_model(inputs, tf.ones((32,3)), tf.constant(True))
        for key in outputs:
            print((key, outputs[key]))
        loss = get_loss(tf.zeros((32,NUM_POINT),dtype=tf.int32),
